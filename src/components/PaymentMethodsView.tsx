...
export default function PaymentMethodsView({ state, onUpdateState }: PaymentMethodsViewProps) {
  ...
  const isModalOpen = !!(editingMethod || isCreateOpen);
  const closeModal = () => { setEditingMethod(null); setIsCreateOpen(false); setFormDirty(false); };
      ...
      {(editingMethod || isCreateOpen) && (
        ...
        <div className="fixed inset-0 bg-foreground/20 backdrop-blur-[2px] flex items-center justify-center z-50 p-4" onClick={attemptClose}>
          <form 
            ...
            className={`bg-card rounded-xl shadow-card-hover border border-border max-w-sm w-full overflow-hidden flex flex-col ${shaking ? 'animate-shake' : ''}`}
          >
